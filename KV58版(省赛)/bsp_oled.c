...
#define LCD_SCL_H GPIOC->PSOR|=0x00004000
#define LCD_SCL_L GPIOC->PCOR|=0x00004000
#define LCD_SDA_H GPIOC->PSOR|=0x00008000
#define LCD_SDA_L GPIOC->PCOR|=0x00008000
#define LCD_RST_H GPIOC->PSOR|=0x00001000
#define LCD_RST_L GPIOC->PCOR|=0x00001000
#define LCD_DC_H  GPIOC->PSOR|=0x00002000
#define LCD_DC_L  GPIOC->PCOR|=0x00002000

/* Define the init structure for the input DC pin */    
gpio_pin_config_t DC_config = {
        kGPIO_DigitalOutput, 1,//0,    
};
/* Define the init structure for the input RST pin */   
gpio_pin_config_t RST_config = {
        kGPIO_DigitalOutput, 1,//1,//0, 
};
/* Define the init structure for the input SDA pin */   
gpio_pin_config_t SDA_config = {
        kGPIO_DigitalOutput, 1,//0,   
};
/* Define the init structure for the input SCL pin */   
...
void LCD_Init(void)        
{
    GPIO_PinInit(GPIOC, 13, &DC_config);
    GPIO_PinInit(GPIOC, 12, &RST_config);
    GPIO_PinInit(GPIOC, 15, &SDA_config);
    GPIO_PinInit(GPIOC, 14, &SCL_config);
...
{ 
        ...
        { 
                c =ch[j]-32;
                if(x>126){x=0;y++;} LCD_Set_Pos(x,y);    
                    for(i=0;i<6;i++)     
                      LCD_WrDat(F6x8[c][i]);  
                      x+=6; 
                      j++; 
...
{ 
          ...
          { 
                for(j=0; j<=79; j++) 
                { 
                      compress_buff[i][j]=ImageBuffer[i][j]; 
                } 
          } 
          for(i=0; i<=59; i++) 
          { 
                for(j=0; j<=79; j++) 
                { 
                      compress_buff[i][j]=compress_buff[i][j]; 
                } 
          } 
          for(i=0; i<=7; i++) 
          { 
                ...
                { 
                      for(k=0; k<=7; k++) 
                      { 
                            bmp_buff[i * 80 + j] += (((*(compress_buff + i * 8 + k))[j]) << k); 
                      } 
                } 
          } 
...
{
	uint8_t i,j,t;
	uint8_t ImageBuffer_col[120][80],ImageBuffer_show[60][80];
	
	for(i=0;i<120;i++)
            for(j=0;j<80;j++)
		ImageBuffer_col[i][j] = ImageBuffer[i][j*2]&&ImageBuffer[i][j*2+1];//行压缩，||保留1（白色）&&保留0（黑色）
	for(i=0;i<60;i++)
	    for(j=0;j<80;j++)
		ImageBuffer_show[i][j] = ImageBuffer_col[i*2][j]&&ImageBuffer_col[i*2+1][j];//列压缩
	
	for(i=0;i<7;i++)//显示80*7*8像素
	{
		LCD_Set_Pos(0,i);
		for(j=0;j<80;j++)
		{
			t=0X00;
			t =ImageBuffer_show[i*8][j]+
			ImageBuffer_show[i*8+1][j]*2+
			ImageBuffer_show[i*8+2][j]*4+
			ImageBuffer_show[i*8+3][j]*8+
			ImageBuffer_show[i*8+4][j]*16+
			ImageBuffer_show[i*8+5][j]*32+
			ImageBuffer_show[i*8+6][j]*64+
			ImageBuffer_show[i*8+7][j]*128;//把一列8个点压在一个字节里
			if(j==39||j==40) t=~t;	       //如果在中垂线则取反
			if(i==3) t=t^0x30;	       //如果在中平行线则取反
			LCD_WrDat(t);
		}
	}
 
	LCD_Set_Pos(0,7);		
	for(j=0; j<80; j++)//显示80*4像素
	{
		t=0X00;
		t=ImageBuffer_show[56][j]+
		ImageBuffer_show[57][j]*2+
		ImageBuffer_show[58][j]*4+
		ImageBuffer_show[59][j]*8;//把一列8个点压在一个字节里
		if(j==39||j==40) t=~t;	  //如果在中垂线则取反
		LCD_WrDat(t);
	}
