...
uint8 ov7725_eagle_reg_init(void)
{
    ...
    SCCB_GPIO_init();
    SCCB_WriteByte (OV7725_COM7, 0x80 ); /*复位sensor */
    OV7725_EAGLE_Delay_ms(50);
    SCCB_ReadByte(&Sensor_IDCode, 1, OV7725_VER ); /* 读取sensor ID号*/

    if(Sensor_IDCode == OV7725_ID)
    {
        for( i = 0 ; i < ov7725_eagle_cfgnum ; i++ )
        {
            SCCB_WriteByte(ov7725_eagle_reg[i].addr, ov7725_eagle_reg[i].val);
        }
    }
    else
    {
